...
async function loadMenu() {
    try {
        ...
        if (!data.data || data.data.length === 0) {
            // Keep static content if no data from API
            return;
        }
        ...
        // Keep static content on error - don't replace with error message
    }
}
        ...
        productsGrid.innerHTML = data.data.map((product, index) => `
            <div class="product-card fade-in-up" style="animation-delay: ${index * 0.1}s">
                ...
                <div class="product-content">
                    ...
                    <p class="product-description">${stripHtml(product.description) || 'توضیحاتی ثبت نشده'}</p>
...
async function loadRecommended() {
    try {
        ...
        recommendedCarousel.innerHTML = data.data.map((item, index) => `
...
async function loadServices() {
    try {
        ...
        servicesGrid.innerHTML = data.data.map((service, index) => {
            ...
            return `
                <div class="service-card fade-in-up" style="animation-delay: ${index * 0.1}s">
                    ...
                    ${service.description ? `<p class="service-desc">${stripHtml(service.description)}</p>` : ''}
                </div>
            `;
        }).join('');
    } catch (error) {
...
async function loadBlogs() {
    try {
        ...
        blogsGrid.innerHTML = data.data.slice(0, 6).map((blog, index) => {
            const cleanContent = blog.content ? stripHtml(blog.content).substring(0, 150) : '';
            return `
            <div class="blog-card fade-in-up" style="animation-delay: ${index * 0.1}s">
                ...
                <div class="blog-content">
                    ...
                    <p class="blog-excerpt">${cleanContent}...</p>
...
function initScrollAnimations() {
    ...
    document.querySelectorAll('.stat-card, .menu-card, .product-card, .service-card, .blog-card').forEach(el => {
        observer.observe(el);
    });
}
